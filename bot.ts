...
async function generateQuizQuestion(previousTopics: string[] = [], previousQuestions: string[] = []) {
...
JSON:
{
  ...
  if (uniqueOptions.size < 4) {
    // Якщо є дублікати — регенеруємо з іншою темою
    return generateQuizQuestion(previousTopics, previousQuestions);
  }
