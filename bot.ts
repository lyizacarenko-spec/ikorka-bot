...
- Щука: 1.5-2.5 мм
- Осетер: 2.5-3 мм (МАЛЕНЬКЕ зерно — чорна ікра!)
- Форель: 4-4.5 мм
- Горбуша: 4-5 мм
- Кижуч: 5-5.5 мм
- Лосось: 5-6 мм
...
async function generateQuizQuestion(previousTopics: string[] = [], previousQuestions: string[] = [], attempt = 0): Promise<any> {
  if (attempt >= 3) {
    throw new Error("Не вдалося згенерувати унікальне питання після 3 спроб");
  }
  ...
  // FIX: використовуємо чергу + retry
  const response = await groqQueue.add(() =>
    withRetry(() => groq.chat.completions.create({
      model: "llama-3.3-70b-versatile",
      max_tokens: 1024,
      messages: [
        {
          role: "system",
          content: `${IKORKA_KNOWLEDGE}${prevQuestionsNote}
...
JSON:
{
  ...
  try {
    ...
    throw new Error("Невалідний JSON від Groq");
  }
...
bot.on("message", async (msg) => {
  ...
  try {
    ...
    // AI ASSISTANT — FIX: черга + retry
    await bot.sendChatAction(chatId as number, "typing");
    const aiResponse = await groqQueue.add(() =>
      withRetry(() => groq.chat.completions.create({
        model: "llama-3.3-70b-versatile",
        max_tokens: 800,
        messages: [
          {
            role: "system",
            content: `Ти — асистент менеджера магазину Ikorka Shop. Відповідай коротко і по суті ТІЛЬКИ українською мовою.\n\n${IKORKA_KNOWLEDGE}\n\nЯкщо питають про фото ікри — давай посилання: https://t.me/+KPwmfo_kSy83Yjhi`,
          },
          { role: "user", content: text },
        ],
      }))
    );
    const aiText = aiResponse.choices[0]?.message?.content ?? "Не зрозумів питання. Спробуйте ще раз.";
...
async function analyzeCall(chatId: number | string, transcript: string) {
  // FIX: черга + retry
  const analysis = await groqQueue.add(() =>
    withRetry(() => groq.chat.completions.create({
      model: "llama-3.3-70b-versatile",
      max_tokens: 1500,
      messages: [
